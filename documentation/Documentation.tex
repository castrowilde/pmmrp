...
В този проект се запознаваме и реализираме няколко метода за обработка на изображения, предложени от Ebihara et.al \cite{imgprocessing}. Използвайки реакционно-дифузен модел (и по-конкретно този на Fitz-Hugh \& Nagumo) при условията на Тюрингова неустойчивост \cite{turinginstability} могат да се намерят ръбове на обекти, да се сегментират изображения, намалява шумът и увеличава контрастът.
...
Искаме разликата на потока през дадена точка $x$ и потока в точка $x+E$ да е равна на изменението в концентрацията във всички точки в интервала $[x,x+E]$ (вж. фигура (\ref{fig:fickslaw})). Така получаваме
...
Решавайки уравнението на дифузията \ref{eq:diffusion} с начално условие изображението от \ref{fig:baseimgdiff}, получаваме \ref{fig:monoimgdiff}. Средната фигура показва една от итерациите, при която обектът още е ясно различим от фона, но дифузията е започнала да премахва ясно дефинираните ръбове. Резултантното изображение е размазано, почти навсякъде сме получили хомогенен нюанс на сивото.
...
%ТУК ДА СЕ ОПИШЕ КАКВО ЩЕ ПРАВИМ ВСЪЩНОСТ
...
\begin{subfigure}[b]{0.3\textwidth}
    ...
    \label{fig:noisemonoimg}
\end{subfigure}
\caption{Резултати от обработката на изображение с повече детайли и фон с повече от един нюанс. Моделът ясно разграничава обекта от фона.\\Източник: https://www.pexels.com/}
\label{fig:noiseimgs}
\end{figure}
